...
int low_level_init(char* filename, int size, int dma_addr, int reg_addr){
    ...
    buffer = (int32_t* ) mmap(NULL, mmap_size, PROT_READ, MAP_SHARED, fd_data, 0);
    if(buffer < 0) {
...
int write_proxied_register(int proxy_base_addr, int addr, int val){
    int offset = (proxy_base_addr - registers_base_addr)/4;
    registers[offset] = val;
...
void read_data(int32_t *data, int size){
    memcpy(data, buffer,size*sizeof(int32_t));
}